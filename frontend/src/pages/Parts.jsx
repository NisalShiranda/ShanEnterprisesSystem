...
const Parts = () => {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            alert(err.response?.data?.message || 'Error creating part');
        }
    };
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Machine Parts</h1>
                    <p className="text-slate-500">Manage your parts and components inventory</p>
                </div>
                <button
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr>
                                    <td colSpan="5" className="text-center py-10 text-slate-500">Loading parts...</td>
                                </tr>
                            ) : filteredParts.length === 0 ? (
                                <tr>
                                    <td colSpan="5" className="text-center py-10 text-slate-500">No parts found</td>
                                </tr>
                            ) : filteredParts.map((part) => (
                                <tr key={part._id} className="hover:bg-slate-50 transition">
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className={`font-bold ${part.stock < 10 ? 'text-red-500' : 'text-slate-700'}`}>
                                            {part.stock}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 font-medium text-slate-700">LKR {part.price?.toLocaleString()}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button className="p-2 text-slate-400 hover:text-primary transition"><Edit2 size={18} /></button>
                                            <button onClick={() => handleDelete(part._id)} className="p-2 text-slate-400 hover:text-red-500 transition"><Trash2 size={18} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
